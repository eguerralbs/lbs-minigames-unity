...
This sequence delivers the landscape, touch-first logic activities in a fixed order, ending with **Apple Math → Lobby**. Use this page as the contributor-level map; implementation references are included only where maintenance requires them.
...
| Apple Math | `apple.math` | Discover the value of one apple cube. | Three apple cubes equal 6, so tap **2**; instruction audio only, no visible text. | Completion, then Lobby |
...
- **BGM:** all games in `LevelSequenceRoute.IsLogicSequenceGame`, including `apple.math`, share the logic-sequence BGM. See `Assets/App/Navigation/LevelSequenceRoute.cs`.
