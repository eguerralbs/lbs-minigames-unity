...
This sequence delivers the landscape, touch-first logic activities in a fixed order, ending with **Shape-Put → Lobby**. Use this page as the contributor-level map; implementation references are included only where maintenance requires them.
...
| Shape-Put | `shape.put` | Identify the third shape in the displayed sequence. | Tap the purple square (Option 2); instruction audio only, no visible text. | Completion, then Lobby |
...
- **BGM:** all games in `LevelSequenceRoute.IsLogicSequenceGame`, including `apple.math` and `shape.put`, share the logic-sequence BGM. See `Assets/App/Navigation/LevelSequenceRoute.cs`.
