...
This sequence delivers the landscape, touch-first logic activities in a fixed order, ending with **Age-Compare → Lobby**. Use this page as the contributor-level map; implementation references are included only where maintenance requires them.
...
| Shortest Route | `shortest.route` | Identify the character with the shortest route. | Tap the koala at the end of the blue route; instruction audio only, no visible text. | Apple Math (`apple.math`) |
| Apple Math | `apple.math` | Discover the value of one apple cube. | Three apple cubes equal 6, so tap **2**; instruction audio only, no visible text. | Shape-Put (`shape.put`) |
| Shape-Put | `shape.put` | Identify the third shape in the displayed sequence. | Tap the purple square (Option 2); instruction audio only, no visible text. | Age-Compare (`age.compare`) |
| Age-Compare | `age.compare` | Listen and choose the asked age. | Tap **1**; instruction audio only, no visible text. | Completion, then Lobby |
...
- **BGM:** all games in `LevelSequenceRoute.IsLogicSequenceGame`, including `shape.put` and `age.compare`, share the logic-sequence BGM. See `Assets/App/Navigation/LevelSequenceRoute.cs`.
